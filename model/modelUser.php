...
class modelUsuario extends PDOConnectionFactory {
    ...
    public function getNovaSenha() {return $this->nova_senha;}
    public function getConfNovaSenha() {return $this->conf_nova_senha;}
    ...
    public function setNovaSenha($nova_senha) {$this->nova_senha = $nova_senha;}
    public function setConfNovaSenha($conf_nova_senha) {$this->conf_nova_senha = $conf_nova_senha;}
    ...
    public function alteraSenha($id_usuario, $senha_atual, $nova_senha, $conf_nova_senha){

        $this->conex = PDOConnectionFactory::getConnection();
        $this->setId($id_usuario);
        $this->setPass($senha_atual);
        $this->setNovaSenha($nova_senha);
        $this->setConfNovaSenha($conf_nova_senha);
        $senha;


        try {

                $stmt = $this->conex->prepare("select * from usuario where id_usuario = ?");
                $stmt->bindValue(1,$this->getId());
                $stmt->execute();
                ...
                if($this->getPass() != $senha){
                    ...
                    if($this->getNovaSenha() != $this->getConfNovaSenha()){
                        ...
                        $stmt = $this->conex->prepare("update usuario set senha = ? where id_usuario = ?");
                        $stmt->bindValue(1,$this->getNovaSenha());

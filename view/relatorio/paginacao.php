...
<?php
        ...
        if (  $_GET['pg']  > 0) { 			
            echo '<a href="#" class=pg onclick="pag('.($_GET['pg'] -1).')"><b>&laquo; anterior </b></a>';
        } else {
            echo "<font color=#CCCCCC>&laquo; anterior </font>";
        }
		if (  $_GET['pg']  > 3) {            
			echo '<a href="#" class=pg onclick="pag(0)"><b>1</b></a> ... ';
		}
        ...
        for($i_pg=$pg;$i_pg<$quant_pg;$i_pg++) {
				...
				if (($i_pg - 1) == ($pg - 1) and ($i_pg - 1) != 0 and ($i_pg != 0) and ($i_pg > 3)) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.($i_pg-4).')"><b>'.($i_pg-3).'</b></a>&nbsp;';													
				}
				if (($i_pg - 1) == ($pg - 1) and ($i_pg - 1) != 0 and ($i_pg != 0) and ($i_pg > 2)) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.($i_pg-3).')"><b>'.($i_pg-2).'</b></a>&nbsp;';										
				}
				if (($i_pg - 1) == ($pg - 1) and ($i_pg - 1) != 0 and ($i_pg != 0) and ($i_pg > 1)) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.($i_pg-2).')"><b>'.($i_pg-1).'</b></a>&nbsp;';
				}
				...
				if (($i_pg -1 < $pg) and ($pg != ($quant_pg-1))) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.$i_pg.')"><b>'.($i_pg+1).'</b></a>&nbsp;';
				}
				if (($i_pg -1< $pg) and ($pg <= ($quant_pg-3))) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.($i_pg+1).')"><b>'.($i_pg+2).'</b></a>&nbsp;';
				}
				if (($i_pg -1 < $pg) and ($pg <= ($quant_pg-4))) {
					echo '&nbsp;<a href="#" class=pg onclick="pag('.($i_pg+2).')"><b>'.($i_pg+3).'</b></a>&nbsp;';
				}
        }	
        //mostra a ultima pagina se não estiver na penultima
		if (( $_GET['pg'] + 5) < $quant_pg) {
			echo ' ... <a href="#" class=pg onclick="pag('.($quant_pg-2).')"><b>'.($quant_pg-1).'</b></a>';
		}
        // Verifica se esta na ultima página, se nao estiver ele libera o link para próxima
        if (( $_GET['pg'] +2) < $quant_pg) { 
                echo '<a href="#" class=pg onclick="pag('.($_GET['pg'] +1).')"><b> próximo &raquo;</b></a>';
        } else { 
